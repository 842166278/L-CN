...
namespace Xerath
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            Config = new Menu("随风-ESK0R泽拉斯", ChampionName, true);
            ...
            Config.SubMenu("R").AddItem(new MenuItem("EnableRUsage", "自动 R").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("rMode", "模式").SetValue(new StringList(new[] { "正常", "有延迟", "按键"})));
            Config.SubMenu("R").AddItem(new MenuItem("rModeKey", "手动 按键").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.SubMenu("R").AddSubMenu(new Menu("延迟 设置", "Custom delays"));
            for (int i = 1; i <= 3; i++)
                Config.SubMenu("R").SubMenu("Custom delays").AddItem(new MenuItem("Delay"+i, "延迟"+i).SetValue(new Slider(0, 1500, 0)));
            Config.SubMenu("R").AddItem(new MenuItem("PingRKillable", "本地提示可击杀敌人").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("BlockMovement", "右键 释放大招").SetValue(false));
            Config.SubMenu("R").AddItem(new MenuItem("OnlyNearMouse", "R鼠标附近的目标").SetValue(false));
            ...
            //Farming menu:
            Config.AddSubMenu(new Menu("打钱 设置", "Farm"));
            Config.SubMenu("Farm")
                .AddItem(
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("AutoEGC", "自动E丨阻止突进").SetValue(true));


            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterR", "显示 3xR 伤害").SetValue(true);
