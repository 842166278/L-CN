...
namespace TwistedFate
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("随风-ESK0R卡牌", "TwistedFate", true);

            var TargetSelectorMenu = new Menu("目标 选择", "Target Selector");
            TargetSelector.AddToMenu(TargetSelectorMenu);
            Config.AddSubMenu(TargetSelectorMenu);

            var SowMenu = new Menu("走砍 设置", "Orbwalking");
            SOW = new Orbwalking.Orbwalker(SowMenu);
            Config.AddSubMenu(SowMenu);

            /* Q */
            var q = new Menu("Q - 设置", "Q");
            q.AddItem(new MenuItem("AutoQI", "自动Q丨无法移动").SetValue(true));
            q.AddItem(new MenuItem("AutoQD", "自动-Q丨突进").SetValue(true));
            q.AddItem(new MenuItem("CastQ", "手动 Q (按键)").SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(q);
            ...
            misc.AddItem(new MenuItem("PingLH", "聊天栏提示低血量目标 (本地通知)").SetValue(true));
