...
namespace Azir
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            #region Menu
            Menu = new Menu("随风-ESK0R沙皇", "Azir", true);
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("UseIgnite", "使用 点燃").SetValue(true));
            Menu.SubMenu("Combo").AddItem(new MenuItem("AllInKEK", "打一套技能 (按键)!").SetValue(new KeyBind('G', KeyBindType.Press)));
            Menu.SubMenu("Combo").AddItem(new MenuItem("ComboActive", "连招 按键!").SetValue(new KeyBind(32, KeyBindType.Press)));

            Menu.SubMenu("Harass").AddItem(new MenuItem("HarassMinMana", "最低蓝量比例 %").SetValue(new Slider(20, 0, 100)));
            Menu.SubMenu("Harass").AddItem(new MenuItem("HarassActive", "骚扰 按键!").SetValue(new KeyBind('C', KeyBindType.Press)));

            Menu.SubMenu("LaneClear").AddItem(new MenuItem("UseQLC", "使用 Q").SetValue(true));
            Menu.SubMenu("LaneClear").AddItem(new MenuItem("UseWLC", "使用 W").SetValue(true));
            Menu.SubMenu("LaneClear").AddItem(new MenuItem("LaneClearActive", "清线 按键!").SetValue(new KeyBind('V', KeyBindType.Press)));

            Menu.SubMenu("Misc").AddItem(new MenuItem("Jump", "跳跃到鼠标位置").SetValue(new KeyBind('E', KeyBindType.Press)));
            Menu.SubMenu("Misc").Item("Jump").ValueChanged += Program_ValueChanged;
            Menu.SubMenu("Misc").AddItem(new MenuItem("AutoEInterrupt", "使用E丨打断目标技能").SetValue(false));

            Menu.SubMenu("R").AddItem(new MenuItem("AutoRN", "自动R命中数").SetValue(new Slider(3, 1, 6)));
            Menu.SubMenu("R").AddItem(new MenuItem("AutoRInterrupt", "使用R丨打断目标技能").SetValue(true));
            ...
            Menu.SubMenu("Drawings").AddItem(new MenuItem("QRange", "Q 范围").SetValue(new Circle(true, Color.FromArgb(150, Color.Yellow))));
            Menu.SubMenu("Drawings").AddItem(new MenuItem("WRange", "W 范围").SetValue(new Circle(false, Color.FromArgb(150, Color.Yellow))));
            Menu.SubMenu("Drawings").AddItem(new MenuItem("RRange", "R 范围").SetValue(new Circle(false, Color.FromArgb(150, Color.Yellow))));
